# -*- coding: utf-8 -*-
"""
Created on Mon Nov 26 01:31:42 2018
...
def rabbit_hole(dict_val, str_val):
    key_list = []
    for i in range(len(dict_val)):
        try:
            key_str = str_val
            key_list.append(key_str)
            str_val = dict_val[str_val]
            print (key_str, str_val)
            if dict_val[str_val] not in dict_val:
                return dict_val[str_val] 
            else:
                if dict_val[str_val] in key_list:
                    return False
        except:
            return str_val
...
a_dict = {'fox': 'jay', 'elk': 'fly', 'jay': 'doe', 'bee': 'elk', 
          'cat': 'dog', 'cow': 'bee', 'pig': 'cat', 'ram': 'rat', 
          'cod': 'hen', 'dog': 'ant', 'ewe': 'cod', 'rat': 'ram', 'bat': 'pig', 'hog': 'fox'}

print(rabbit_hole(a_dict, "rat"))
